...
it("can add the star name and star symbol properly", async () => {
  ...
  let user1 = accounts[1];
  let starId = 6;
  await instance.createStar("totally chill star", starId, { from: user1 });

  //2. Call the name and symbol properties in your Smart Contract and compare with the name and symbol provided
  let contractName = await instance.tokenName.call();
  let contractSymbol = await instance.symbol.call();

  assert.equal(contractName, "Limitless Star Coin");
  assert.equal(contractSymbol, "LSC");
});
...
it("lets 2 users exchange stars", async () => {
  ...
  const starId1 = 7;
  const starId2 = 8;
  await instance.createStar("Test Star 1000", starId1, { from: user1 });
  await instance.createStar("Test Star 1001", starId2, { from: user2 });
...
it("lets a user transfer a star", async () => {
  ...
  const user2 = accounts[1];
  const starId = 9;
  await instance.createStar("Test Star 1000000", starId, { from: user1 });
...
it("lookUptokenIdToStarInfo test", async () => {
  ...
  const user1 = accounts[0];
  const starId = 10;
  await instance.createStar("Coolest Star Ever", starId, { from: user1 });

  // 2. Call your method lookUptokenIdToStarInfo
  const starName = await instance.lookUpTokenIdToStarInfo(starId);

  // 3. Verify if you Star name is the same
  assert.equal(starName, "Coolest Star Ever");
});
